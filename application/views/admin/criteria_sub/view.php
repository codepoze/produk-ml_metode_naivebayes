            ...
            <div class="page-body">
                <div class="card">
                    ...
                    <div class="card-block table-border-style">
                        <table class="table table-striped table-bordered nowrap" id="tabel-criteria_sub-dt" style="width: 100%;">
                        </table>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="modal-add-upd" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            ...
            <form id="form-add-upd" action="<?= admin_url() ?>criteria_sub/save" method="POST">
                <!-- begin:: id -->
                <input type="hidden" id="<?= $this->security->get_csrf_token_name() ?>" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>" />
                <input type="hidden" name="id_criteria_sub" id="id_criteria_sub" />
                <!-- end:: id -->

                <div class="modal-body">
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">Criteria *</label>
                        <div class="col-sm-9">
                            <select class="form-control" name="id_criteria" id="id_criteria">
                                <option value="">Pilih Criteria</option>
                                <?php foreach ($criteria->result() as $row) : ?>
                                    <option value="<?= $row->id_criteria ?>"><?= $row->nama ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
