...
if (empty($_SESSION['id_users'])) {
	...
	// untuk data user
	$sqlLog = "SELECT u.id_users, u.nama, u.email, u.username, m.tgl_lahir, m.tmp_lahir, m.kelamin, m.telepon, m.alamat FROM tb_member AS m LEFT JOIN tb_users AS u ON u.id_users = m.id_users WHERE u.id_users = '$id'";
	$qryLog = $pdo->Query($sqlLog);
	$rowLog = $qryLog->fetch(PDO::FETCH_OBJ);
}
